...
const uuid = () => {
    ...
    const _uuid = temp_url.toString();
    URL.revokeObjectURL(temp_url);
    return _uuid.substring(_uuid.lastIndexOf('/') + 1);
}
/**
 * 实现扁平化
 */
...
const getParentEle = (ele: any, parent: string, count = 8): any => {
    ...
    const conditionFn =
        (dom: HTMLElement, _parent: string) => (is(_parent[0], '.') && is(typeOf(dom.className), 'string') && dom.className.includes(str)) ||
            (is(_parent[0], '#') && is(dom.id, str)) ||
            is(dom.tagName, _parent.toUpperCase());
...
const distinguishFileTypes = (fileUrl: string) => {
    if (fileUrl) {
        ...
        const imageSuifx = ['jpg', 'png', 'gif', 'jpeg', 'bmp']
        const wordSuifx = ['doc', 'docx']
        const excelSuifx = ['xls', 'xlsx']
        const pdfSuifx = ['pdf']
        const videoSuifx = ['mp4', 'avi', 'wmv', 'mpg', 'mpeg', 'mov', 'rm', 'ram', 'swf', 'flv', 'rmvb', 'asf', 'divx', 'mpe', 'mkv', 'vob']
        const audioSuifx = ['cda', 'wav', 'mp3', 'wma', 'ra', 'midi', 'ogg', 'ape', 'flac', 'aac']
        const typeMap = [
            [imageSuifx, 'image'],
            [wordSuifx, 'word'],
            [excelSuifx, 'excel'],
...
const downloadOrPreview = (type: string, file: any, cb: any) => {
    if (type === 'download') {
        ...
        requestX.onload = (e) => {
            const _url = window.URL.createObjectURL(requestX.response)
...
const diffEnvFun = (types: string[], cb: any) => {
    ...
    if (types.some(x => process.env.NODE_ENV === obj[x])) {
        cb && cb()
    }
}
