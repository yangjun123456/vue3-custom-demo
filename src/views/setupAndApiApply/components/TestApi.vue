<template>
    <div class="test-api-body"
        ...
        <div class="toRef"
            ...
            <div>toRefState.name === {{ toRefState }}</div>
        ...
        <div class="toRefs"
            ...
            <button @click="toRefsClick1">toRefsClick1</button>
            <div>toRefsState.name === {{ toRefsState.name }}</div>
            <div>toRefsState2.age 解构赋值 === {{ age }}</div>
            <div>toRefsState2.age 非解构赋值 === {{ toRefsState2.age }}</div>
            <div>toRefsState2.toRefsObj2Value 解构赋值获取值 === {{ toRefsObj2Value }}</div>
            <div>toRefsState2.toRefsObj2Value 非解构赋值获取值 === {{ toRefsState2.toRefsObj2Value }}</div>
...
const TestApi = defineComponent({
  ...
  setup(props, context) {
    ...
    const refClick1 = () => {
      count.value++;
      console.log(count, obj); // 不会修改原始值，会更新dom，如果有其它的操作更新dom，也不会更新原始值
    };
    const refClick2 = () => {
      bol.value = !bol.value;
      console.log(bol);
    ...
    // ref类似深拷贝，toref类似浅拷贝
    const toRefObj1 = { name: 'yang', age: 28, teach: 'teacher. yang', ff: { m: 0, val: { count: 1 } } };
    const toRefState = toRef(toRefObj1, 'name'); // 会修改原始数据，不会更新到dom
    // const toRefState = ref(toRefObj1.name); // 不会修改原始数据会更新到dom，如果是引用类型，会修改原始数据不会更新到dom
    timer(3000).subscribe(() => {
      toRefState.value = 'yuan';
      console.log('toRefState====', toRefObj1, toRefState);
    });

    const toRefobj = { count: 3 };
    const toRefcount = toRef(toRefobj, 'count');
    const toRefClick1 = () => {
      toRefcount.value++;
    ...
    const toRefsObj1 = { name: 'yang', age: 28, teach: 'teacher. yang', ff: { m: 0, val: { count: 1 } } };
    const toRefsState = toRefs(toRefsObj1); // 会修改原始数据，不会更新到dom
    timer(3000).subscribe(() => {
      toRefsState.name.value = 'yuan';
    ...
    const toRefsState2 = toRefs(toRefsObj2); // 会修改原始数据，不会更新到dom
    // const { toRefsObj2Value } = toRefsObj2; // 解构赋值失去响应式
    const { toRefsObj2Value, age } = toRefsState2; // // 解构赋值不会失去响应式
    const toRefsClick1 = () => {
      toRefsState2.age.value++;
      console.log('toRefsState 用法2 传入的是响应式对象=====', toRefsObj2, toRefsState2);
    };
    timer(3000).subscribe(() => {
      toRefsState2.toRefsObj2Value.value = 'yuan';
