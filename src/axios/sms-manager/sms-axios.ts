...
import { getToken, getAuthorization } from '@/axios/auth'

// 使用.env.development里面定义的api
// const apiBaseUrl = '/managerSms';
const apiBaseUrl = process.env.VITE_APP_SMS_MANAGER;

class HttpRequest { // 定义一个接口请求类，用于创建一个axios请求实例
  constructor(public baseUrl: any = apiBaseUrl) { // 这个类接收一个字符串参数，是接口请求的基本路径
  ...
  private interceptors(instance: AxiosInstance, url?: string) { // 定义这个函数用于添加全局请求和响应拦截逻辑
    ...
    instance.interceptors.response.use((res: AxiosResponse) => {
      ...
      if (res.status !== 200) {
        ...
        if (res.data === 'token失败' && res.status === 200) {
        ...
        //   }
          ...
          if (res.data?.code) {
            if (res.data?.code === 200) {} else {
              console.error(res.data.msg) // 如果不是0，则打印错误信息，我们后面讲到UI组件的时候，这里可以使用消息窗提示
            }
          } else {
